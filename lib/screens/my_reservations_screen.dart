...
class _MyReservationsScreenState extends State<MyReservationsScreen> {
  ...
  Map<String, List<ReservationModel>> _groupByDate(List<ReservationModel> reservations) {
    ...
    for (final r in reservations) {
      ...
      if (date.isAfter(today)) {
        groups['Próximos']!.add(r);
      } else if (date == today) {
        groups['Hoy']!.add(r);
      } else if (date == yesterday) {
        groups['Ayer']!.add(r);
      } else if (date.isAfter(lastWeek)) {
        groups['Última semana']!.add(r);
      } else if (date.isAfter(lastMonth)) {
        groups['Último mes']!.add(r);
      } else {
        groups['Más antiguo']!.add(r);
      }
    }
      ...
      body: SafeArea(
        child: Consumer2<AuthProvider, ReservationProvider>(
          builder: (context, auth, provider, _) {
            ...
            final groups = _groupByDate(provider.reservations);
